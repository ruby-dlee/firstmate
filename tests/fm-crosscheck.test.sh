...
(repo / "tests" / "test_private_dependency.py").write_text(
    ...
    "    try:\n"
    "        Path(path).write_text('write escaped', encoding='utf-8')\n"
    "    except PermissionError:\n"
    "        return\n"
    "    raise AssertionError(f'sandbox allowed writing {path}')\n"
    "\n"
    ...
    "write_control = Path('.crosscheck/proof-write-control')\n"
    "write_control.write_text('control wrote', encoding='utf-8')\n"
    "assert write_control.read_text(encoding='utf-8') == 'control wrote'\n"
    "denied_write('.crosscheck/python-cache/operator-cache-secret')\n"
    "denied_write('.crosscheck/python-env/pyvenv.cfg')\n"
    "try:\n"
