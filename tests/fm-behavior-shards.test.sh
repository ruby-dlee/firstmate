...
test_checked_in_plan_is_complete_balanced_and_deterministic() {
  ...
  assert_contains "$out" "FM_BEHAVIOR_PLAN ok tests=94 shards=8" \
    "coverage guard did not report the complete 94-test inventory"
...
test_post_run_guard_requires_the_exact_executed_union() {
  ...
  assert_contains "$out" "FM_BEHAVIOR_COMPLETENESS ok tests=94 shards=8" \
