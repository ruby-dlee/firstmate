...
FM_CREW_STATE_NM_TIMEOUT=10   # seconds allowed per no-mistakes query or active-run backend agent probe inside fm-crew-state.sh
FM_CREW_STATE_GH_TIMEOUT=10   # seconds allowed per GitHub PR-state or remote-currentness query inside fm-crew-state.sh
FM_CREW_STATE_RUNS_LIMIT=200  # recent no-mistakes runs rows scanned for the metadata-bound task branch when axi status reports another branch
