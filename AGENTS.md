...
This repo's configured commands `exec bin/fm-nm-command-supervisor.sh`, and `bin/fm-nm-step-liveness.sh` reports `alive` or `dead` only from that exact PID, process group, start identity, and worktree; cwd presence without the supervisor record is `unknown`, never working (`docs/postmortems/nm-quiet-test-step.md`).
The supervisor makes a missing command return a real failed step through no-mistakes' existing wait path, so never abort a quiet run to recover it.

- `running`/`fixing`/`ci` - the pipeline is working (a fix round, a test, or CI monitoring), except that a quiet configured command without exact live supervisor identity is unknown; `ci` stays working until the ci log's most recent recognized marker says checks passed or no checks are terminally ready, and a later re-arm or issue marker returns it to working.
