...
test_checked_in_plan_is_complete_balanced_and_deterministic() {
  ...
  assert_contains "$out" "FM_BEHAVIOR_PLAN ok tests=87 shards=8" \
    "coverage guard did not report the complete 87-test inventory"
...
test_post_run_guard_requires_the_exact_executed_union() {
  ...
  assert_contains "$out" "FM_BEHAVIOR_COMPLETENESS ok tests=87 shards=8" \
...
test_teardown_partition_preserves_every_full_suite_case() {
  ...
  [ "$(wc -l < "$listed" | tr -d ' ')" -eq 107 ] \
    || fail "teardown partition does not retain all 107 normal-run cases"
  ...
  [ "$(awk 'NR % 2 == 1 { count++ } END { print count + 0 }' "$listed")" -eq 54 ] \
    || fail "teardown partition A does not own exactly 54 cases"
  ...
  pass "teardown wrappers preserve all 107 normal cases and three focused-only cases"
}
