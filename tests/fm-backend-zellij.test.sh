...
test_forced_secondmate_teardown_kills_zellij_children_with_child_home_tag() {
  local dir state data config home project child_worktree fixture_remote source_default_tip source_checkout fb out status child_title
  ...
  # Teardown verifies this fixture against the live upstream default, so seed the
  # private remote from that same identity rather than from a possibly stale
  # remote-tracking ref or this feature checkout's HEAD.
  source_default_tip=$(git -C "$ROOT" ls-remote origin refs/heads/main | awk 'NR == 1 { print $1 }')
  [ -n "$source_default_tip" ] || fail "secondmate fixture could not resolve live origin/main"
  if ! git -C "$ROOT" cat-file -e "$source_default_tip^{commit}" 2>/dev/null; then
    git -C "$ROOT" fetch -q origin "$source_default_tip" \
      || fail "secondmate fixture could not fetch live origin/main"
  fi
  ...
  # upstream probe resolves against the local fixture bare, not the moving real
  # origin: a CI checkout is a snapshot, and comparing its cached default tip
  # against the live remote fails whenever main advances between checkout and
  # this test.
  source_checkout="$dir/source-checkout"
  git clone -q --no-local "$fixture_remote" "$source_checkout"
  git init -q "$project"
  git -C "$project" -c user.name=fmtest -c user.email=fmtest@example.invalid \
    commit -q --allow-empty -m initial
  mkdir -p "$(dirname "$child_worktree")"
  git -C "$project" worktree add -q -b childz "$child_worktree"
  printf '{"worktrees":[{"name":"1","path":"%s","leased":true,"lease_holder":"firstmate-childz"}]}\n' \
    "$child_worktree" > "$dir/treehouse-pool/treehouse-state.json"
...
if [ "${1:-}" = list-sessions ]; then
  ...
  if [ -f "$FM_ZELLIJ_LOG.closed" ]; then
    printf '[]\n'
  else
...
elif [[ "$*" == *'close-tab-by-id'* ]]; then
  : > "$FM_ZELLIJ_LOG.closed"
fi
SH
...
#!/usr/bin/env bash
target=$(pwd -P)
git -C "$FM_ZELLIJ_CHILD_PROJECT" worktree remove --force "$target"
SH
  chmod +x "$dir/fakebin/treehouse"
  fb="$dir/fakebin"
  out=$( PATH="$fb:$PATH" FM_STATE_OVERRIDE="$state" FM_DATA_OVERRIDE="$data" FM_CONFIG_OVERRIDE="$config" \
    FM_ROOT_OVERRIDE="$source_checkout" \
    FM_PROJECTS_OVERRIDE="$dir/source-projects" \
    FM_ZELLIJ_LOG="$dir/log" FM_ZELLIJ_CHILD_TITLE="$child_title" FM_ZELLIJ_CHILD_PROJECT="$project" \
