...
test_send_text_submit_detects_landed_send() {
  ...
  assert_contains "$(cat "$log")" $'\x1f''agent'$'\x1f''send'$'\x1f''w1:p2'$'\x1f''hello captain' "send_text_submit did not type the literal text first"
...
test_send_text_submit_send_failed() {
  ...
  pass "fm_backend_herdr_send_text_submit: reports 'send-failed' when both literal-send paths error"
}
