...
The open-decision fold is one linear awk stream pass; rebuilding its accumulated state through nested Bash command substitutions is forbidden because a 1.25 MB long-lived secondmate log reproduced a full-core child and stopped the parent watcher inside its first fleet pass.
In that shape the recorded watcher root remains live and sleeping while Bash waits on the runaway descendant, and TERM is deferred behind that wait; this is the observed live-lock/stale-beacon wedge, not an ordinary actionable wake-and-exit cycle.
...
`bin/fm-watch-arm.sh` remains the verified arm wrapper for protocols that call it; it forks the watcher as a tracked child, verifies it is genuinely alive with a progress beacon inside `FM_WATCH_PROGRESS_GRACE` or an identity-bound unexpired phase deadline, and prints an honest initial status (`started` / `attached` / restart-only `healthy` / `FAILED`, the last exiting non-zero).
A live lock and a beacon inside the broad 300-second handoff grace are not enough for attachment: the tighter progress bound defaults to 60 seconds, so an already-sick 155-second-old holder is refused and directed through exact-tree restart instead of being called healthy until the broad threshold expires.
`FM_POLL` is the terminal wait after a fleet pass, not a fixed start-to-start period: endpoint reads and classification add to it.
The watcher therefore refreshes its progress beacon between bounded fleet phases and per-window reads, so a large healthy fleet keeps the full stale-grace margin instead of publishing only once per complete pass.
Configured sleeps, checks, account synchronization, report retention, and the complete automatic teardown path publish identity-bound phase deadlines, so `FM_POLL` and Treehouse's return bound cannot be mistaken for a wedge while an expired phase still fails the tighter cadence check.
An actionable wake is deliberately one-shot: `wake()` queues the reason and exits, the lock is released by the normal EXIT trap, and the tracked arm completion wakes the primary harness; the primary drains that queued reason and starts the successor cycle.
The brief no-lock interval after such an exit is expected handoff state rather than a crash, and the durable queue plus the arm's reason output distinguish it from unexplained disappearance.
...
Its `--restart` mode stops only the watcher recorded in the current home's `state/.watch.lock` and descendants proven by numeric parentage, escalating from TERM to KILL after a bound, so restarting one home cannot kill sibling secondmate watchers and can recover a TERM-resistant wedge without manual pid surgery.
While waiting on a started or attached cycle, the arm samples that exact recorded tree and makes sustained full-core consumption actionable; the started-cycle owner also reaps the tree, while an attachment reports the supported `bin/fm-watch-arm.sh --restart` recovery command.
A pull-based guard (`bin/fm-guard.sh`) warns through supervision tool output if the primary checkout is tangled, if tasks are in flight and that watcher stops running or a live lock misses the tighter progress cadence or queued wakes are waiting to be drained, or if the exact lock-recorded watcher tree is consuming a core despite a fresh beacon.
...
`bin/fm-backend.sh` centralizes backend selection, `state/<id>.meta` helpers, selector resolution, and operation dispatch; `bin/backends/tmux.sh` is the verified reference adapter ([`docs/tmux-backend.md`](tmux-backend.md)), `bin/backends/herdr.sh` (P2), `bin/backends/zellij.sh` (P3), and `bin/backends/cmux.sh` (P5) are experimental new-task spawn adapters, and `bin/backends/orca.sh` (P4) remains an experimental legacy-task lifecycle adapter.
...
The deeper session-start agent-process liveness probe is separate from that busy-state poll; [`docs/configuration.md`](configuration.md#runtime-backend-configbackend--fm_backend) owns the backend-specific verdicts.
...
Independently, every directly invocable fleet, project, X-mode, watcher, wake-drain, report-stack, and merge mutator uses `bin/fm-gate-refuse-lib.sh`'s boundary and exits with status 3 before mutation when the gate environment marker is present or the current checkout matches the default no-mistakes gate-repository topology.
A normal primary checkout or crewmate worktree has neither signal and remains unaffected.
The helper's header owns the exact guarded-entrypoint decision list, signal detection, relocated-home limitation, test-harness bypass, and relationship to no-mistakes' HEAD-continuity guard.
...
The [`operating-fundamentals`](../.agents/skills/operating-fundamentals/SKILL.md) skill owns the intake and orchestration contract for actionable requests, while fleet-state questions get one answer in the current turn.
Actionable requests get an acknowledgement reply first; `bin/fm-x-link.sh` records `x_request=`, `x_request_ts=`, `x_followups=0`, and optional reply-platform context in the tracked task's `state/<id>.meta`, while durable per-request context preserves the original platform and budget independently of task links and inbox cleanup.
