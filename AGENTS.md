...
`bin/fm-pr-merge.sh` always records `pr=` and records `pr_head=` when available before merging, runs `bin/fm-crosscheck.sh run <id> <PR URL>` as the fail-closed independent adversarial review gate, parses the full `https://github.com/<owner>/<repo>/pull/<n>` URL into `gh-axi pr merge <n> --repo <owner>/<repo>`, and defaults to `--squash` unless an explicit merge method is forwarded after `--`; this holds even on a repo with no PR CI where the "checks green" signal that normally triggers `bin/fm-pr-check.sh` never fires - do not call `gh-axi pr merge` directly for a task's PR, or the recording and crosscheck steps can be silently skipped and a later `fm-teardown.sh` has nothing to verify a squash merge against.
crosscheck stores a durable finding ledger under `data/<id>/crosscheck-ledger.json`; old findings remain blocking until a later run verifies the fix with mutation proof, so silence from a later reviewer never closes them.
It reviews in a scratch checkout and does not touch the author/no-mistakes worktree, so it may be run early in parallel with no-mistakes for signal, but the merge helper still runs it at merge time against the current PR head.
