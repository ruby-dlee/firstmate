...
test_capture_calls_pane_read() {
  ...
  out=$( PATH="$fb:$PATH" FM_HERDR_LOG="$log" FM_HERDR_RESPONSES="$resp" \
    bash -c '
      ...
      fm_backend_herdr_capture default:w1:p2 250
    ' "$ROOT" )
...
test_capture_works_around_small_lines_bug() {
  ...
  out=$( PATH="$fb:$PATH" FM_HERDR_LOG="$log" FM_HERDR_RESPONSES="$resp" \
    bash -c '
      . "$0/bin/backends/herdr.sh"
      fm_backend_herdr_server_adapter_owned() { return 0; }
      fm_backend_herdr_capture default:w1:p2 2
    ' "$ROOT" )
...
test_current_path_reads_cwd() {
  ...
  out=$( PATH="$fb:$PATH" FM_HERDR_LOG="$log" FM_HERDR_RESPONSES="$resp" \
    bash -c '
      ...
      fm_backend_herdr_current_path default:w1:p2
    ' "$ROOT" )
...
test_composer_state_bare_prompt_is_empty() {
  ...
  out=$( PATH="$fb:$PATH" FM_HERDR_LOG="$log" FM_HERDR_RESPONSES="$resp" \
    bash -c '
      ...
      fm_backend_herdr_composer_state "$1"
    ' "$ROOT" default:w1:p2 )
