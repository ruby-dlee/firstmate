...
Poll-path absorption is gated instead on two proofs taken from one immutable read of the crewmate's current durable status stream: the pause verb carrying no failure vocabulary in its headline, and an empty keyed open/resolved fold, so a pause cannot mask a still-unanswered decision on that path.
The Herdr native blocked-transition edge currently applies only the last-line pause-verb test; [`herdr-backend.md`](herdr-backend.md) owns that path's verified behavior.
