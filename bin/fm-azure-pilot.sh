...
require_cloud_environment() {
  ...
  if [ -n "${FM_AZURE_WORKER_SKUS:-}" ]; then
    ...
    WORKER_SKUS=Standard_D4as_v6,Standard_D4as_v6,Standard_D4as_v7,Standard_D4as_v7,Standard_D4s_v6,Standard_D4s_v6,Standard_D4ads_v7,Standard_D4ads_v7,Standard_D4ds_v6,Standard_D4ds_v6,Standard_D4s_v7,Standard_D4s_v7,Standard_D4ds_v7,Standard_D4ds_v7,Standard_D4ads_v6,Standard_D4ads_v6
  elif [ "$CAPACITY_PROFILE" = commissioning ]; then
    WORKER_SKUS=Standard_D4as_v6,Standard_D4as_v6
  else
    WORKER_SKUS=
  fi
  RUNNER_VALIDATION_SKU=${FM_AZURE_RUNNER_VALIDATION_SKU:-Standard_E8as_v6}
...
expected = {
    ...
    9: "Standard_D4ds_v6", 10: "Standard_D4ds_v6",
    11: "Standard_D4s_v7", 12: "Standard_D4s_v7",
    13: "Standard_D4ds_v7", 14: "Standard_D4ds_v7",
    15: "Standard_D4ads_v6", 16: "Standard_D4ads_v6",
}
  ...
  jq -e \
    ...
    --argjson workers "$WORKER_SKUS_JSON" '
      ...
      | length == ($selected | length)
        ...
        and any(.[]; .name == $runner and .vcpu >= 8 and .memory >= 32)
...
PY
  ...
  if [ "$COMMAND" = worker-create ]; then
    ...
    [ "$regional_free" -ge 10 ] || refuse "commissioning requires at least 10 free East US Total Regional vCPUs"
  else
...
values = {
    ...
    "runnerValidationSku": os.environ.get("FM_AZURE_RUNNER_VALIDATION_SKU", "Standard_E8as_v6"),
...
sku_for_slot() {
  ...
  case "$1" in
    ...
    9|10) printf 'Standard_D4ds_v6\n' ;;
    11|12) printf 'Standard_D4s_v7\n' ;;
    13|14) printf 'Standard_D4ds_v7\n' ;;
    15|16) printf 'Standard_D4ads_v6\n' ;;
    *) return 1 ;;
  esac
}
...
run_destroy() {
  ...
  az group delete \
    ...
    --output none \
    --only-show-errors
  printf 'destroy submitted after compute quiescence, session revocation, state export, lock removal, and explicit retained-disk deletion; Key Vault purge protection remains in force\n'
}
