...
Multi-file judgment publication records a durable before-image journal before replacing any destination, removes that journal only after every destination and the data directory are durable, and reconciles an interrupted transaction under the shared writer lock before a later capture, compact recovery, or ordinary memory writer can proceed.
The suite kills the publisher after its first destination replacement and verifies that the next capture restores the complete prior generation before rendering its anchor.
