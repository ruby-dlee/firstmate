...
if hasattr(module, "_stage_author_repository"):
    ...
    # The stacked predecessor predates guest staging; retain equivalent focused
    # payload checks without importing unrelated lifecycle changes into this fix.
    held = [entry["commit"] for entry in repository["preserved_refs"]]
    context = guest / ".fm-context"
    context.mkdir()
    subprocess.run(
        ["tar", "-xf", str(staging / "context.tar"), "-C", str(context)], check=True
    )
