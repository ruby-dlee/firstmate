...
# flight together once a PR exists. `verify` re-reads live GitHub state, requires
# the latest attempt for that exact head and claims document to be clear, and
# prints only the reviewed SHA. `merge` is retained as a recognized command but
# refuses before the private GitHub primitive so no repository route can arm
# future execution.
set -eu
...
if [ "${1:-}" = merge ]; then
  echo "error: crosscheck merge execution is disabled because an immediate GitHub request can arm a merge queue; no merge API call was made" >&2
  exit 1
fi
