...
# report:
#   (a) no process in the worktree            -> unknown, never death
...
# --- (a) no process in the worktree -> unknown ------------------------------

out=$("$PROBE" "$RUN_ID" --worktree "$WT" --sample 0)
[ "$(verdict_of "$out")" = unknown ] || fail "empty worktree must read unknown, got: $out"
assert_contains "$out" "procs: 0" "absence evidence reports zero observed processes"
assert_contains "$out" "absence does not prove idle, death, or a wedge" \
  "absence evidence omitted the non-destructive predicate"
pass "no process with its cwd in the worktree remains unknown"
...
[ "$(verdict_of "$out")" = unknown ] || fail "layout resolution must find the empty worktree without inferring death, got: $out"
...
# genuinely inactive one produced the same reading and three healthy runs were
# aborted on it. These cases pin the only safe asymmetric distinction over real
# processes: affirmative activity may prove BUSY, while every absence remains
# UNKNOWN and can trigger only non-destructive investigation.
...
out=$(cat "$GAP_OUT")
# The invariant is that a gap is never reported as death. It is deliberately
# not asserted to be `alive`: with the successor only just observed there is no prior sample to
...
# (j4) Even repeated confirmed absence remains UNKNOWN because host scheduling
# and subprocess turnover have no proved observation-gap upper bound.
out=$(FM_NM_ABSENCE_CONFIRM_DELAY=1 "$PROBE" "$RUN_ID" --worktree "$WT" --sample 0)
[ "$(verdict_of "$out")" = unknown ] \
  || fail "REGRESSION: repeated absence must remain unknown, got: $out"
assert_contains "$out" "across two scans" "the unknown verdict records the repeated window"
assert_contains "$out" "absence does not prove idle, death, or a wedge" \
  "the repeated absence verdict omitted its epistemic limit"
pass "regression: repeated absence remains unknown and cannot authorize cancellation"
