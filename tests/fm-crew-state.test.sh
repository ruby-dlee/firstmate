...
test_quiet_step_downgrades_legacy_dead_to_unknown() {
  reset_fakes
  ...
  assert_contains "$out" "liveness: unknown" "a quiet step with no process remains epistemically unknown"
  assert_not_contains "$out" "liveness: dead" "legacy dead vocabulary escaped the consumer downgrade"
  assert_contains "$out" "step: test" "the liveness observation names its active step"
  pass "a quiet step downgrades a legacy dead probe to unknown"
}
...
test_quiet_step_probe_failures_are_unknown() {
  ...
  assert_contains "$out" "liveness: unknown (grade: unreadable; 2 procs" \
    "a legacy dead verdict with processes is downgraded without an inactivity claim"
  assert_not_contains "$out" "liveness: dead" \
    "legacy dead vocabulary escaped the consumer downgrade"
...
EOF
  ...
  cat > "$d/fakebin/fm-run-liveness.sh" <<'SH'
#!/usr/bin/env bash
printf 'alive: run=bbbbbbb samples=2 interval=1s counts=1,1 cpu=1,2 cpu_delta=1s repo_test_baseline=unknowns\n'
SH
  chmod +x "$d/fakebin/fm-run-liveness.sh"
  PATH="$d/fakebin:$PATH" FM_STATE_OVERRIDE="$d/state" \
    FM_RUN_LIVENESS_BIN="$d/fakebin/fm-run-liveness.sh" crew_is_provably_working feat-provable \
