...
The R4 receipted-close acceptance (task `r4-cell-acceptance`, validation generation `azv-accept-260820-a`) was submitted through branch `fm/r4-cell-acceptance` after PR #267 scoped the shard-receipt wipe to run boundaries and bound collected receipts to the cell's own dispatched shard round; the isolated cell validating this paragraph is itself that acceptance run, driven from submit through dispatch, drive, observe, collect, and a receipted `close` with its worktree disk released.
