...
A Pi author additionally records the exact selected provider slot in its model prefix, and `fm-spawn.sh` snapshots that slot's `accountId` into task metadata at the initial launch boundary.
Same-provider eligibility requires that recorded identity and never re-reads ambient Pi credentials as author proof; legacy or otherwise missing snapshots fail closed.
...
The gate detects the project's declared Node major, selects a matching interpreter from the standard version-manager directories, and materializes dependencies afresh from exactly one tracked `package-lock.json` with `npm ci --offline --ignore-scripts` or tracked `pnpm-lock.yaml` with `pnpm install --offline --frozen-lockfile --ignore-scripts` inside the no-network proof sandbox.
The selected Node path remains bound through dependency installation and the baseline and mutated Jest runs.
A cold dependency cache, missing or ambiguous lockfile, preexisting runner, unavailable package manager or Node version, unsupported Vitest route, or missing materialized Jest binary is `CANNOT-CERTIFY` and never a test verdict.
...
Its `test_preexisting_jest_runner_cannot_certify` case proves that a committed Jest-shaped output script is refused before package-manager materialization, and `test_jest_runs_under_declared_node_major` proves the selected Node path governs installation and both proof executions.
