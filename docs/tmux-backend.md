...
Pick tmux unless you have a specific reason to try a new-task-capable experimental backend (herdr, zellij, or cmux) - it is the fully verified reference path for secondmate homes, while cmux does not support secondmate spawns and Orca is legacy-recovery-only.
...
`fm_tmux_composer_state` sends a bare shell glyph on an unbordered row to the shared composer classifier as `unknown`, so automated delivery cannot mistake it for an empty agent composer; see [Composer-emptiness safety](herdr-backend.md#composer-emptiness-safety-2026-07-10-fleet-wide-across-all-four-backends).
...
A second case matters for a harness that shells out to subcommands while it runs (git, npm, test runners, and similar tools): does `pane_current_command` report the harness or the subcommand?
