...
if [ "$KIND" = secondmate ]; then
  ...
  if [ "$ACCOUNT_EFFECTIVE_MODE" = enforce ]; then
    if ! secondmate_home_supports_account_routing "$PROJ_ABS"; then
      echo "error: refusing account-routed secondmate $ID launch for $PROJ_ABS: the home lacks Agent Fleet routing support. Fast-forward or otherwise reconcile the home, including any dirty working tree, to this Firstmate revision, run bin/fm-config-push.sh, and retry." >&2
      exit 1
    fi
