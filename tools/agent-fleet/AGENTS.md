...
- Agent Fleet is machine-global and provider-neutral.
  Firstmate is a client, not a dependency.
- Work only from a feature-branch worktree.
  Never edit, commit, or push `main` directly.
...
- JSON is the shell integration contract.
  TOON is the default agent-facing structured output.
- Tests must use temporary homes and fake provider/quota binaries.
  They must not touch real provider homes.
