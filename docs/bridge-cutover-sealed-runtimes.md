...
It runs only before the worker-state transaction begins, while its journal, snapshot, and snapshot-staging paths are all absent.
Once any of those transaction artifacts exists, refresh refuses rather than restarting, concluding, cleaning up, or otherwise mutating the transaction, and the current driver has no action that returns a begun transaction to this absent-artifact state.
