...
`prepare_bridge_cutover.py refresh <bundle>` refreshes exactly those two identity artifacts of that existing bundle in place to the current provider-binary identity, while leaving the sealed cutover and sealed-adoption journals untouched.
It holds the worker-state advisory lock across the journal check and both atomic artifact replacements, so worker-state cannot begin against the old fingerprint during refresh.
It runs only while the worker-state journal is absent, which is the driver's exact `not-started` state, and refuses any bundle with a worker-state transaction already bound to the current manifest fingerprint.
It also refuses recoverable worker-state snapshot or staging artifacts because those remain bound to the current fingerprint even before journal publication.
It never restarts or mutates an existing snapshotted transaction, so a bound worker-state transaction must be concluded or reset back to `not-started` before refreshing.
If refresh is interrupted after replacing the bundle but before replacing the worker-state manifest, rerunning refresh repairs the stale manifest and restores an exact pair.
It also refuses unless the bundle is provably the applied `runtime-switched` one - it reuses the same journal-bound phase determination as validation - and it gates completion on a full strict `validate_bundle` that must still report the `runtime-switched` phase, so it makes the strict gates pass only because the recorded identity again equals the live binary, never by loosening any check.
