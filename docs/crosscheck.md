...
Its `test_pytest_requirements_supplement_is_reconstructible_and_narrow` case proves that an empty dependency-free pyproject with no uv lock selects a tracked requirements file instead of ambient pytest, while pyproject dependencies, dependency groups, uv development dependencies, uv locks, and ordinary nested-project resolution retain the normal route.
...
Its `test_real_pytest_coverage_policy_outcomes_fail_closed` case invokes locked real pytest 9.1.1 and proves that the measured coverage-policy path accepts a passing baseline, refuses a genuine baseline failure, keeps a coverage-only mutation blocking, and refuses output without authoritative test counts.
