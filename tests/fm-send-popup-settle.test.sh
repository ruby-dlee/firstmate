...
# for a leading `$<skill>` invocation (e.g. `$checkpoint-task`). Submitting before the
...
first_settle() {  # <expected> <label> <harness|--explicit> <message> [selector-form]
  ...
  if [ "$harness" = --explicit ]; then
    ...
    # The recorded window must carry the task's own fm-<id> label, exactly as
    # fm-spawn.sh records it: the adapter verifies that the window a target
    # resolves to really is this task's, so a fixture recording someone else's
    # label is not a valid stand-in for a live task endpoint.
    fm_write_meta "$home/state/$meta_id.meta" "window=sess:fm-$meta_id" "harness=$harness"
  fi
  : > "$log"
...
first_settle 1.2 'codex $skill -> long settle' codex '$checkpoint-task'
...
first_settle 1.2 'codex $skill exact task id -> long settle' codex '$checkpoint-task' exact

# Same `$` message to claude keeps the fast path: `$` is ordinary text there.
first_settle 0.3 'claude $-message -> fast path' claude '$checkpoint-task'
...
first_settle 0.3 'explicit target $message -> fast path (unknown harness)' --explicit '$checkpoint-task'
