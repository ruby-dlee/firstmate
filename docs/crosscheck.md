...
After installation, it requires the package identity and version to match the lock entry and requires `node_modules/.bin/jest` to resolve to the executable `bin/jest.js` inside that real materialized package directory.
