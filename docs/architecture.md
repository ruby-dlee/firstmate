...
The step-liveness probe owns the CPU-versus-elapsed measurement and threshold: `alive` means worktree processes measurably advanced CPU or process membership, while `stalled` means their progress remained at the near-zero floor.
Only `alive` refreshes the bounded wedge observation; `stalled`, `dead`, and `unknown` remain actionable, so process presence alone never suppresses a wedge.
...
A durable declaration also explains the expected idle state after a matching no-mistakes run has reached current-state `done` or `parked`; active, failed, stale, and unknown runs retain run-step precedence, and stopped crewmates without a valid declaration surface immediately.
