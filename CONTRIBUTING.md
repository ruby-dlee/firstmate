...
Every change reaches `main` through a pull request with green CI and an exact-head Crosscheck review performed by a configured maintainer.

## Workflow

1. Fork the repo, then clone your fork and retain the parent repository as an upstream remote.
2. Create a branch and make your changes.
3. Run the focused tests for the changed behavior plus `bin/fm-lint.sh` when shell files change.
...
5. Open a pull request against `main` and keep ownership of any CI corrections.
6. A configured maintainer runs the exact-head Crosscheck review after CI is green and merges only the reviewed SHA.
...
- Only shared material is tracked: `AGENTS.md`, `README.md`, `CONTRIBUTING.md`, `.tasks.toml`, `.github/workflows/`, `bin/`, `.agents/skills/`, `skills/`, and provider-neutral components under `tools/`.
  ...
  Everything personal to one captain's fleet (`.env`, `data/`, `state/`, `config/`, and `projects/`) is gitignored; never commit it.
...
- Helper scripts in `bin/` are plain bash.
  ...
  `bin/fm-lint.sh` must pass: it is the single owner of the lint definition used by local verification and CI.
...
Changes to the shared tracked material listed under "Repo conventions" ship through a feature-branch pull request, green CI, exact-head Crosscheck, and explicit merge approval.
...
Crewmates run focused project tests before pushing and own every CI correction.
Firstmate runs Crosscheck only after the PR head is stable and CI is green; a new push invalidates the verdict.
[`docs/crosscheck.md`](docs/crosscheck.md) owns the reviewer and exact-head merge contract.
