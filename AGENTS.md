...
Use `gh-axi` for all GitHub operations and `chrome-devtools-axi` for all browser operations; route captain-facing decisions through `lavish-decision-boards`, and use `lavish-axi` directly for reports complex enough to deserve a rich review surface.
...
9. Surface only what needs the captain: route pending decisions through `lavish-decision-boards`, and directly surface PRs ready to merge, failures, or needed credentials.
...
Route every captain-facing no-mistakes decision ask through `lavish-decision-boards`.
...
- Review findings that need the captain's decision, routed through `lavish-decision-boards` unless routine approval is authorized on firstmate judgment.
...
Route captain-facing decisions through `lavish-decision-boards`; use `lavish-axi` directly for structured reports worth a visual.
...
- `lavish-decision-boards` - load before presenting any captain-facing decision ask or creating or revising a Lavish board that collects structured feedback.
