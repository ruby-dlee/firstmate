...
`bin/fm-watch.sh` splices this in as the watcher's terminal wait (`event_wait_or_sleep`, replacing the blind `sleep POLL` for push-capable homes): on a returned `blocked` it resolves one exact task from safe Herdr metadata, acquires that task's lifecycle lock, revalidates the exact task-to-window mapping, exempts `kind=secondmate` endpoints and waits whose current status line has the `paused:` verb, and enqueues an immediate `stale` wake otherwise.
The wake and native-transition commit share that lifecycle lock, working-edge and teardown clears use the same boundary, and every commit or clear revalidates ownership after acquisition so teardown and window reuse cannot recreate retired state.
