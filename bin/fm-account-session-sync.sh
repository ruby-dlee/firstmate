...
# validates task/profile/provider/pool stickiness before appending the real
# provider_session_id= to meta.
...
while :; do
  if json=$("$binary" --format json session status --task "$ID" 2>/dev/null); then
    break
  fi
  [ "$(date +%s)" -lt "$deadline" ] || {
    if [ "$REQUIRE" = 1 ]; then
      echo "error: no Agent Fleet provider-session mapping for managed task $ID; refusing recovery" >&2
    fi
    exit 1
  }
  sleep 1
...
[ "$mapped_task" = "$ID" ] || { echo "error: Agent Fleet session task mismatch for $ID" >&2; exit 1; }
