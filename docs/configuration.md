...
That command requires `tmux` on `PATH`, prints `tmux -V`, runs every behavior test through `tests/run.sh`'s sealed per-test home, and fails if any script exits non-zero.
