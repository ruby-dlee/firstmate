...
Behavior tests must place a fixed per-script bound on background process fan-out while retaining their full assertion and input matrix.
`tests/fm-wake-queue.test.sh` keeps all 40 concurrent-queue append operations but permits at most five simultaneous test-script processes: the root, three append workers, and one drain worker.
Its `FM_TEST_FOCUSED=concurrency-bound` regression injects worker delay to simulate a slow host, records every process enter and exit under a serialized counter, and asserts the bound continuously while comparing a small historical unbounded sample with the bounded scheduler.
