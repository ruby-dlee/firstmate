...
# through a backend busy signal, and surfaced
...
# while the crewmate shows positive evidence it is still working (a busy pane,
# via crew_is_provably_working over
...
# new entry during a long tool call, so the log can retain a stale terminal
# event. On a new hash, give a busy pane (the same
