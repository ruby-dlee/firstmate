...
FM_CREW_STATE_BIN=bin/fm-crew-state.sh   # test override for the current-state reader used by working/paused/captain-owed watcher triage
...
FM_PAUSE_RESURFACE_SECS=3600       # seconds before an idle declared wait re-surfaces; waits on open keyed decisions are labelled captain-owed, never wedge
