...
The Jest executable, environment, runner, test framework, runtime, and measured Circus run and invocation modules must resolve through the exact Jest 29.7.0 executable package graph, both modules must match their measured SHA-256 digests, and none may resolve inside the proof checkout.
...
The Vitest parent starts with a gate-owned `NODE_OPTIONS` preload that captures trusted primitives before tracked configuration, removes the preload from the native environment before project code runs, and locks only the exact Vitest fork-worker launch boundary.
That launch boundary rejects project-provided worker options and injects the exact gate preload into each worker, while the final gate plugin rechecks that tracked configuration and plugin resolution left no ambient `NODE_OPTIONS` and Node's native string-coercing `process.env` object remains unchanged.
...
A transform forgery, resolved-config mutation, custom-environment forgery, primitive replacement, unattested worker configuration, ambient worker preload, dropped class-plugin hook, or second runner construction is classified as `NON-EXECUTION`.
...
Its real Jest and Vitest integration cases retain tracked setup, aliases, and plugins while injecting body evidence, preserve Jest callback and callable-property semantics plus Node's native environment coercion, then require tracked mutation-only startup failures, hook body replacement, generator tests, an early Jest channel-capture attempt, forged Circus dispatch, project-local and sibling Jest runtime spoofs, Vitest primitive, transform, and resolved-config mutations, ambient or unattested workers, custom environments, and class-plugin startup failures to remain `NON-EXECUTION`.
