...
Terminal signals and merged-PR checks first enter `bin/fm-auto-reap.sh`, which excludes secondmates, validates only the terminal trigger, snapshots any exactly attributed no-mistakes run, and delegates every destructive safety decision to ordinary `bin/fm-teardown.sh`.
Only after teardown succeeds does it abort that exact no-mistakes run so its monitor and pipeline agent are collected without touching the shared daemon.
If that post-teardown abort cannot be verified, the exact run and process identity remain in a durable cleanup marker for the next supervision scan.
Both successful collections and fail-closed refusals become actionable watcher output, while unchanged refusals are rate-limited until task state changes or the bounded retry interval arrives.
