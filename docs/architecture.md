...
`bin/fm-crew-state.sh <id>` is the cheap current-state read for an actionable heartbeat review: it binds a no-mistakes ship to metadata's exact `worktree_git_ref`, uses run-step evidence only for a unique `refs/heads/fm/<id>` self-binding, refuses missing, ambiguous, or uniquely disagreeing identity, and falls back to pane/status-log evidence with an explicit warning whenever another live task record shares the branch.
For an attributable run, run-step remains authoritative even if the pane has closed; a shared branch is never lane-level run evidence, and the pause precedence above remains the watcher's absorb classification rather than a change to attributable run precedence.
