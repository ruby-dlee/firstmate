...
The 2026-08-28 production run `01M151DHQQBWBHN2SEKJMM1ZDE` established the bounded-parallel inventory baseline before the later six-reader and scoped-lint cutovers.
...
The first ordinary post-cutover review, run `01M157FJ7HKYMNYMFD2H9Z7GJX` at exact repository head `d729fd0978fe100c5f270e0c97568813fe172f51`, exercised the six-reader inventory path and completed admission in 216,383 ms, execution in 125,450 ms, and cleanup in 148,227 ms, for 490,060 ms across the three phases with zero exact-slot residue.
That run was cancelled through the owner-fixes-only path after its reviewer found that the earlier wording overstated what this evidence proved, so it is review-phase evidence rather than an end-to-end validation result.
...
`abandon-claim` can record and clear an exact-key provider result whose apply deterministically refuses, including a script-bound Failed or Canceled execution disposition that names the claimed task-command resource; an exact-key `REFUSED-IDENTITY` replay whose recorded resource identity can never bind again; or an execute that two fresh, unchanged Azure views prove never started while the VM is deallocated or stopped and the assignment blobs are still initial.
The never-started proof tolerates a non-succeeded sibling monitor or bootstrap child without weakening ordinary create or execute readiness, but it still refuses incomplete or conflicting resource identity, execution evidence, substantive command source, staging drift, changed observations, or a non-exact durable marker.
The never-started exit durably marks the exact key and every future execute refuses that marker; ordinary dark-VM cleanup then cascades the empty Run Command child.
