...
test_peek_send_and_crew_state_route_through_orca_meta() {
  ...
  if PATH="$FB:$PATH" FM_ORCA_LOG="$LOG" FM_ORCA_RESPONSES="$RESP" \
    ...
    "$ROOT/bin/fm-send.sh" "fm-$id" "hello orca" >/dev/null 2>&1; then
    fail "fm-send admitted Orca split pane steering"
  fi
  ...
  assert_not_contains "$(cat "$LOG")" $'orca\x1f''terminal'$'\x1f''send' \
    "fm-send atomic refusal still reached Orca terminal input"
  pass "fm-peek/fm-crew-state use Orca metadata while fm-send refuses split steering"
}
