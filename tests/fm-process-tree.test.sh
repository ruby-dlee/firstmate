...
with tempfile.TemporaryDirectory() as directory:
    fifo_guard = os.path.join(directory, "guard.fifo")
    os.mkfifo(fifo_guard)
    fifo_arguments = marker + b"\0FM_PROCESS_TREE_GUARD_FILE=" + os.fsencode(fifo_guard)
    assert module._guard_retains_process(fifo_arguments, 50)
