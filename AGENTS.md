...
- `outcome: passed` or `checks-passed` - the helper cross-checks the actual GitHub PR state for `passed` and reports merged, open, closed-without-merge, or unavailable rather than deriving PR state from the pipeline outcome, while `checks-passed` means it is ready for PR review.
  An open PR reports `done` only when the remote-currentness contract owned by `bin/fm-crew-state.sh`'s header succeeds; `unknown` or `stale` always means `do not merge`.
...
`bin/fm-watch.sh` classifies every wake in bash and absorbs the benign majority without waking you: crewmates with positive attendance evidence from a busy pane read via `bin/fm-crew-state.sh` unless the separate permission-stall no-progress threshold has expired, a declared `paused:` external wait with no active or parked validation until its bounded recheck cadence (including the stale-pane-only terminal-run exception owned by `docs/architecture.md`), and no-change heartbeats.
An active no-mistakes run is work state, not attendance evidence; a turn-end, pause, or stale pane with only that run state surfaces for inspection.
