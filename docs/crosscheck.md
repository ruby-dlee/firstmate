...
When available, the ledger's optional `telemetry.review_process.stage_metrics` records challenge and synthesis elapsed time, turn counts, token classes, declared cost, and aggregate retrieval counts. Retrieval includes read/search calls, per-stage unique and repeated paths/ranges, returned bytes, and truncations; it never stores paths, queries, or source. Historical records and lookup-followup runs without these fields remain readable.
`retrieval.complete=false` means an unsuccessful pre-repair attempt had no replayable tool log; the retained counts remain valid lower bounds for successfully replayed attempts.
Review guidance is selected from the merge-base root and nearest applicable `AGENTS.md`/`REVIEW.md`/path instruction files, deduplicated and bounded, and remains untrusted guidance rather than proof.
