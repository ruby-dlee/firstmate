...
Nothing about the child's home, account, worktree, harness, SKU, project, or repository is expressible from the cloud side; the project is local policy (`FM_SECONDMATE_CHILD_PROJECT`, else the secondmate home's single project), and `FM_AZURE_WORKER_STATE_DIR` stays pinned to the local controller so the money authority remains one document even though `FM_HOME` moves to the secondmate home for the spawn.
...
Child results return over that same lane: a compartment asks with a closed `fm.secondmate-attach-request/v1` carrying no selector at all, and the monitor bundles the commits its secondmate home worktree gained over the compartment's dispatched repository generation, uploads them as `session/in/attach/<sha256>.bundle`, and announces name, digest, and byte count in an inbox message the guest's fetch checks size-first.
