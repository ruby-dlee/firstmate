...
Identity-claim creation acquires the bounded key's identity-mutation lock before publishing the exact task/name claim and releases it before the raw compatibility fence, while bounded dead-owner reclaim holds an already-owned raw fence before acquiring the identity-mutation lock and retains mutation custody through claim pinning, lock quarantine, post-move validation, and contained deletion.
