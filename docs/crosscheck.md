...
`.credentials.json` is handled by credential source: when that file is the verified source it is read through a symlink to the account's own store, so the OAuth secret is never duplicated into the checkout and a refresh write through that link fails closed under the sandbox; when the selected source is the scoped Keychain it is omitted entirely, so the seeded home never carries a file the gate just rejected.
Any other symlink to a non-directory is reproduced as a symlink rather than copied, so a linked entry such as `CLAUDE.md` is not duplicated into the checkout either.
...
On 2026-08-07 the same machinery was re-exercised end to end through `bin/fm-crosscheck.sh run` against a real pull request, with task metadata declaring a `codex` + `gpt-5.6-sol` author, so reviewer selection had to reject every same-model Codex entry and fall through to `claude-opus-5`.
The same live reviewer was run twice, differing only in whether it had a working command-execution tool: with it, the gate recorded a `blocking` verdict whose execution proof names the executed command, its exit status, and a hashed receipt, and whose `execution_home` resolves inside the review checkout; with the tool removed, the gate refused the review as `unreviewed` and exited nonzero rather than accepting a static-only verdict.
That run also retired a previously unproven assumption: the reviewer account's `.credentials.json` was an empty-token stub, so the gate selected the scoped Keychain and the seeded home omitted that file entirely, and installed Claude still authenticated and executed.
