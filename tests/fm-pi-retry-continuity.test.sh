...
out=$(env -u FM_TEST_RUNNER_ACTIVE PATH="$selected_node:$PATH" FM_PI_PACKAGE_DIR="$pi_package" FM_PI_RETRY_CONTINUITY_PROOF=1 "$ROOT/tests/fm-pi-watch-extension.test.sh") || {
  printf '%s\n' "$out" >&2
  exit 1
}
printf '%s\n' "$out" | grep -Fqx 'PI_RETRY_CONTINUITY_PROOF_OK' || {
  printf '%s\n' "$out" >&2
  echo "not ok - Pi retry continuity proof token missing" >&2
  exit 1
}
