...
It runs only while worker-state is at `not-started` and refuses any bundle with a worker-state transaction already bound to the current manifest fingerprint.
