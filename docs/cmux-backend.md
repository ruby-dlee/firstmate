...
You do not need to bring the window forward for routine supervision: from an active firstmate session, `bin/fm-peek.sh <id>` reads a task's surface without focusing it, while canonical text steering currently refuses because cmux has no atomic agent-session-bound submit - workspace/surface/pane creation all default `focus` to `false`, so an unattended spawn never steals your view.
...
| Legacy send + submit helper | `send` then `send-key enter` | cmux has no atomic agent-session-bound submit; the retained helper composes two calls for spawn-time probes and regression tests, while canonical `fm-send.sh` refuses it for steering. |
...
## Legacy composer verification: structural border-row classification
...
`tests/fm-backend-cmux.test.sh` pins this exact regression shape (`test_send_text_submit_popup_autocomplete_requires_second_enter`) for the retained helper.
Canonical `fm-send.sh` refuses before this split transport, so the helper's verdict vocabulary does not constitute production delivery confirmation.
...
5. Before Gate B, `fm-send.sh` accepted a plain-text steer through the structural helper and the next peek showed it landed; this remains historical adapter evidence, not an admitted current route.
6. Historical verification exercised `fm_backend_cmux_send_text_submit` against the popup-placeholder/second-Enter regression class, but canonical `fm-send.sh` no longer admits that split literal-plus-Enter transport as verified steering.
...
- **A surface can still die in the brief window between `target_ready` succeeding and the operation's own call running.** Spawn-time probes remain protected by `fm-spawn.sh`'s worktree-discovery timeout; canonical steering refuses before the legacy submit helper, so this race cannot be reported as current text delivery.
