...
`bin/fm-watch.sh` classifies every wake in bash and absorbs the benign majority without waking you: crewmates with positive working evidence (including the measured CPU/process-progress verdict owned by `docs/architecture.md`), a valid owned-and-clearing `paused:` external wait until its bounded recheck cadence, and no-change heartbeats.
...
`docs/architecture.md` owns the full `paused:` owner/clearing contract; its initial signal surfaces once, incomplete declarations receive no absorption, and a forgotten valid pause re-surfaces once per window.
