...
At the same time, an independent no-mistakes run-step attribution fallback could miss this crewmate's branch when `axi status` reported another branch; current `bin/fm-crew-state.sh` binds the task branch to metadata's exact `worktree_git_ref=refs/heads/fm/<id>` and falls back to top-level `no-mistakes runs --limit ${FM_CREW_STATE_RUNS_LIMIT:-200}` for that coarse cross-branch verdict.
...
The cross-branch attribution fallback now uses the real `no-mistakes runs` command with that metadata-bound branch, and the watcher checks provably-working evidence before a stale status-log verb can make a stale pane terminal.
