...
The tracked `.no-mistakes.yaml` keeps test evidence outside the repo and defines `commands.test` through `bin/fm-no-mistakes-test-command.sh`; that helper preserves the complete local suite by default and owns the explicitly selected Azure/Linux plus local-Herdr split described in `docs/azure-runner.md`.
...
FM_CROSSCHECK_UV_BIN=uv     # uv executable used only for lock-backed Python proof dependency preparation
FM_CROSSCHECK_UV_CACHE_SOURCE=  # canonical resolved uv cache used only during no-network preparation; unset asks installed uv for its cache directory
FM_CROSSCHECK_PYTHON_ENV_MAX_BYTES=2147483648  # maximum copied private Python proof-environment bytes (1 MiB to 8 GiB)
FM_CROSSCHECK_PYTHON_ENV_MAX_FILES=250000      # maximum copied private Python proof-environment entries (100 to 1,000,000)
FM_CROSSCHECK_PYTHON_CACHE_MAX_BYTES=2147483648  # maximum private Python cache-seed bytes (1 MiB to 8 GiB)
FM_CROSSCHECK_PYTHON_CACHE_MAX_FILES=250000      # maximum private Python cache-seed entries (100 to 1,000,000)
