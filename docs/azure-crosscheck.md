...
The primary review family is a registered cross-family lane on the fleet's own Azure AI Foundry resource, driven by Pi as that lane's deployment on its own custom provider slot. Today's registry is `azure-kimi` (`Kimi-K2.7-Code`), `azure-deepseek` (`DeepSeek-V4-Pro`), and `azure-glm` (`FW-GLM-5.2`, retained as evidence and unusable on this subscription: see R6 in docs/azure-requirements.md).
...
`effective_provider_host` is model-aware: a cross-family review derives its own lane's host, today `aif-fm7c799d-eus01.cognitiveservices.azure.com`, as its single egress host and refuses a conflicting configured `provider_host`, while the codex-family fallback keeps its `chatgpt.com` derivation.
