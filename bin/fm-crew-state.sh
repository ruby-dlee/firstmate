...
#      A matching live ci monitor remains authoritative even after a green PR is
#      deliberately held and even when its worktree is missing: monitor-until-merge
#      is still an active, potentially unbounded run rather than an idle lane.
...
if [ "$HAVE_RUN" = 1 ]; then
  ...
  # mode this exists to end. The ci path is excluded because its monitoring runs
  # inside the daemon with no worktree process at all, so `dead` or a missing
  # worktree there is meaningless rather than evidence that the monitor ended.
