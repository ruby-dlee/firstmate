...
Pi's cursor-only empty composer is the third recognized shape: an ANSI row containing only a reverse-video cursor cell (SGR 7 plus whitespace) counts as empty only when Herdr's native agent record independently reports idle/done/blocked; absent native proof remains `unknown`.
...
It keeps `fm_backend_herdr_composer_state` as a structural classifier for the composer's own row - located as the bottom-most bordered composer row, verified bare prompt row, or native-idle-corroborated reverse-video cursor-only row described above - and reports `empty`, `pending`, or `unknown`.
