...
FM_POLL=15              # terminal wait seconds after each watcher fleet pass; scan/read time is additional
...
FM_WATCH_CPU_LIMIT=80   # exact recorded watcher-tree CPU percentage that counts as a hot sample
FM_WATCH_CPU_POLL=5     # seconds between watcher-tree CPU samples in fm-watch-arm
FM_WATCH_CPU_SAMPLES=3  # consecutive hot samples before the arm reports and recovers a runaway
...
FM_WATCHER_STALE_GRACE=300   # defaults to FM_GUARD_GRACE; broad stale-lock error threshold for direct fm-watch duplicate starts (the arm uses FM_WATCH_PROGRESS_GRACE)
