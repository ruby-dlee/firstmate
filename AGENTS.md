...
That contract has exactly two non-success outcomes: a CAPABILITY GAP - a component this provisioner was never able to provision, such as an over-budget component, an unsupported or undeterminable package manager, an unfindable runtime, a missing installer, or a declaration larger than the library traverses - is named on stderr, in `state/<id>.provision.log`, in `provision=` metadata, and in the git-excluded `.fm-provisioning.md` at the root of the leased worktree, which is the only one of the four the crewmate itself can read, and launches the lane with that component unprovisioned; a FAILURE - an attempt that was made and did not complete - refuses the spawn.
