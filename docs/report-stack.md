...
Its namespace is derived from the configured stack path but lives outside that path and every Firstmate home, so it is established before the stack root is opened, typed, validated, pinned, repaired, or locked.
The marker's exclusive creation is the failure-safety boundary, and the current bucket remains present even when stack-local cadence also rejects the attempt, so failures and root-generation swaps cannot reopen the bucket to the next watcher or LaunchAgent loop.
The stack-local `.retention-attempt.json` record remains a recovery record beneath that boundary, while `.retention-attempt.claim.json` is recognized only to retire serializer state left by older generations.
Both local paths classify absent entries, malformed JSON, schema mismatches, non-integral or negative timestamps, future timestamps, and stale timestamps through the same recovery state machine during compatibility recovery.
...
The root-independent marker is installed before any local claim cleanup, and the local canonical record is then atomically installed and verified by inode and timestamp before an old claim is quarantined.
Every installation, verification, validation, or cleanup failure therefore surfaces only after a durable current-bucket admission exists.
...
The mutation inventory is derived mechanically from every atomic condition in each changed JavaScript and Python branch predicate relative to the benchmark base, excluding only explicit test-hook branches.
For every derived file and line, the focused check builds and executes the condition's logical deletion mutant, using false inside disjunctions and true inside conjunctions, against the same positive and negative contract, refuses surviving or unexecuted mutants, and prints its observed red failure.
