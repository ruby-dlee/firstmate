...
The task home also decides WHERE the spawn stages the plaintext provider credential (`<task home>/state/<id>.cloud-account/auth.json`), so it decides where every remover has to look, and the removers run with `FM_HOME` on the primary because the controller document has exactly one home: resolving that home instead removed a path that never existed and left the credential in the compartment home with nothing that would ever take it. Removal therefore does not resolve at its call sites at all - `bin/fm-cloud-state-lib.sh` owns both the resolution and the removal, so the stager and the remover cannot disagree. The stager records the home it staged into at `<controller state>/<id>.cloud-task-home`, next to the one document every remover already reads; a record only redirects into an absolute, traversal-free, non-symlinked directory still carrying the `.fm-secondmate-home` marker the stager itself required, so a remover can never walk into a directory it does not own, and it is removed with the state it points at. The ordinary crewmate lane has task home equal to `FM_HOME`, writes no record, and removes exactly what it removed before the split existed.
