...
The primary review family is a registered cross-family lane, driven by Pi as that lane's model on its own custom provider slot. Today's registry is the single lane `fireworks-glm` (`accounts/fireworks/models/glm-5p2`, GLM-5.2 direct from Fireworks). The Azure Foundry partner lane it replaced is unusable on this subscription: see R6 in docs/azure-requirements.md.
...
The Pi model guest applies the same byte-pinned verdict-body parser as the local gate: a bare JSON object or exactly one complete fenced JSON object with brace-free surrounding prose is accepted, while unterminated fences, multiple complete blocks, brace-bearing remainder, malformed JSON, and non-objects fail closed. The guest independently retains the terminal stream contract too: at least one turn, a final assistant `stop`, and exactly one completed agent are required. Pi's explicit `auto_retry_start` may open a continuation only after a completed attempt executed a turn and did not stop successfully; the continuation starts with empty terminal state and must execute its own turn before completing. `model_guest_pi_verdict_unit` executes the exact shipped heredoc and byte-compares its pure fence parser to the host copy so the self-contained image artifact cannot drift back to `json.loads(final)`. When Pi itself exits zero with a safe terminal stream but the artifact is malformed, non-object, or lacks a dictionary `verdict` or `evidence_files`, the guest permits exactly one protocol-correction attempt in the same credentialed compartment. It deletes the first result and event files, prepends a fixed trusted correction that requires silent reasoning and `{` as the first output byte, then appends the untouched original prompt so its exact schema remains last. No first-output byte enters that prompt, and a nonzero Pi exit or provider/stream failure never earns this correction. Each invocation's stderr stays in a separate attempt file and is never emitted raw; every surfaced stderr or artifact failure summary is control-escaped and byte-bounded, and the common wrapper gate still rechecks the accepted result.
