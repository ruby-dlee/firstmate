...
Nothing about the child's home, account, worktree, harness, SKU, project, or repository is expressible from the cloud side; the project is local policy (`FM_SECONDMATE_CHILD_PROJECT`, persisted into the compartment environment at spawn time because the monitor pane runs in a closed environment, else the secondmate home's single project), the harness is named explicitly as the cloud lane's only runtime rather than resolved implicitly (a home carrying `config/crew-dispatch.json` refuses an implicitly-resolved crewmate harness), and `FM_AZURE_WORKER_STATE_DIR` stays pinned to the local controller so the money authority remains one document even though `FM_HOME` moves to the secondmate home for the spawn.
...
OPEN, and blocking compartment children until it is resolved in the controller: a compartment child must be a secondmate-owned author request, so its spawn runs with `FM_HOME` set to the secondmate home; the child bounds require the parent compartment's queue entry in the same controller document, which is the primary's; and the controller document is bound to the home that created it, so a request made from the secondmate home against the primary's document refuses with `lifecycle state home_binding binding is not exact`.
Until that contract is settled, a compartment child request reaches the controller and is refused there, the refusal is delivered into the compartment inbox as a durable answer, and no child is recorded.
