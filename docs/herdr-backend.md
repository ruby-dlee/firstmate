...
The per-session launch lock lives at `/tmp/firstmate-herdr-server-locks-<uid>` regardless of `TMPDIR`; its physical parent ancestry is validated and its leaf must remain a current-user `0700` directory.
...
Config and certificate publication use private `*.candidate.<pid>` files. Recovery removes only a current-user, single-link, exact-mode candidate whose numeric owner PID is proven absent and whose age exceeds the startup stale threshold; foreign, live, malformed, or otherwise indeterminate candidates are never deleted.
...
The regression suite covers hostile `PATH`, Perl/loader variables, unsafe ancestry, hardlinks, cached-leaf mutation, unsafe lock parents, hostile `TMPDIR`, a detached descendant that retains a safe user tool while excluding a writable search directory, inherited `BASH_ENV` plus `SHELLOPTS=xtrace`, exact managed-config shape, certificate liveness, helper/config replacement and source-update drift, guarded stale-candidate recovery, and refusal of every uncertified production backend before Fleet, lease, or endpoint mutation.

`fm_backend_herdr_server_ensure` deliberately reuses a healthy existing session instead of replacing it. If that session predates this release or its certified helper/config no longer matches the installed release, the subsequent certificate gate refuses routing and instructs the operator to stop the server only after proving it has no tasks or panes; the next Firstmate spawn then relaunches it through the certified path. Firstmate never auto-restarts an occupied or uncertified server, and captain launchers remain outside the Herdr lifecycle.
