...
That interposed source records at the canonical invocation through a preload-captured `Reflect.apply`, leaving the original function object, arity, generator identity, callback behavior, and project-visible properties unchanged rather than trusting publicly dispatchable lifecycle events or project-mutable callable properties.
...
After tracked configuration and plugin resolution, the final gate plugin rejects ambient `NODE_OPTIONS`, replaces the parent environment with a locked detached proxy that refuses later `NODE_OPTIONS` writes, and only then permits worker creation.
...
A transform forgery, resolved-config mutation, custom-environment forgery, unattested worker configuration, ambient worker preload, dropped class-plugin hook, or second runner construction is classified as `NON-EXECUTION`.
...
Its real Jest and Vitest integration cases retain tracked setup, aliases, and plugins while injecting body evidence, preserve Jest callback and callable-property semantics, then require tracked mutation-only startup failures, an early Jest channel-capture attempt, forged Circus dispatch, project-local and sibling Jest runtime spoofs, Vitest transform and resolved-config mutations, ambient or unattested workers, custom environments, and class-plugin startup failures to remain `NON-EXECUTION`.
