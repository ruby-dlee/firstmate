...
MESSAGE=$(cat <<'EOF'
[lavish-only] Built-in structured questions are disabled in firstmate sessions. For a multi-option decision, assemble the complete request.md from one or more checked item wrappers using the exact boundaries in the fm-captain-item-check.sh header; lavish-axi create refuses unchecked or outside prose. Write the ordered questions to questions.json using [{"key":"lowercase-slug","prompt":"Which option?","options":[{"value":"option-a","label":"Option A"},{"value":"option-b","label":"Option B"}]}], then run: lavish-axi create --id <lowercase-slug> --title '<title>' --request <request.md> --questions <questions.json> --destination data/<id>/captain-answer.toon. When the captain is not choosing anything and wants to comment on material instead, do NOT invent questions: write items.json using [{"key":"lowercase-slug","title":"<plain-language item>","body":"<the item text>"}] and pass --items <items.json> in place of --questions, which renders one comment box per item plus an overall note. For a yes/no, ask in plain chat instead. Surface only the decision title and: Run: lavish answer <id>
EOF
) || exit 0
