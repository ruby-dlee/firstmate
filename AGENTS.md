...
state/               volatile runtime signals; gitignored
  ...
  .lock              per-home session lock written by fm-lock.sh: harness PID on line 1, holder process start time on line 2, home= for the canonical FM_HOME, and optional backend=/target= fields only when the holder process identity owns that endpoint; direct readers parse only the first-line PID, while acquire/status treat it as held only when that PID is live with a matching start time (skipped for a legacy one-line lock, which is held while its PID is a live harness) and is not a Codex app-server; home-bound visible delivery revalidates the endpoint against the holder identity and never falls back to ambient terminal state
