...
test_watcher_self_evicts_on_lock_takeover() {
  ...
  fakebin="$dir/fakebin"
  out="$dir/watch.out"
  # Keep this lifecycle fixture on the poll path it is asserting.
  # First-cycle report retention and account-session maintenance are separately
  # bounded, but under a congested full-suite run they can legitimately outlast
  # this test's self-eviction deadline before the watcher reaches its next poll.
  ...
  [ "$(cat "$state/.watch.lock/pid" 2>/dev/null || true)" = "$pid" ] &&
    [ -e "$state/.last-watcher-beat" ] || fail "watcher did not finish publishing its first live cycle"
