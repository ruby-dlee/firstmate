...
- `operating-fundamentals` - load while supervising under load, deciding whether to dispatch or hold, handling a blocked lane or finished crew, protecting shared validation capacity, acting on an explicit captain order, or about to assert a fleet fact.
